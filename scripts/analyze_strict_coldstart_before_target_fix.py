...
def model_rankings(
    ...
    mashup_embeddings, api_embeddings = (
        trainer._prepare_ranking_embeddings(
            mashup_embeddings,
            api_embeddings,
        )
    )
    ...
    use_dot = (
        scorer_name.lower() == "dot"
        or "dot" in trainer.scorer.__class__.__name__.lower()
    )

    for start in range(0, len(target_mashup_ids), batch_size):
        ids = [int(x) for x in target_mashup_ids[start:start + batch_size]]
        idx = torch.tensor(ids, dtype=torch.long, device=device)
        mashup_batch = mashup_embeddings[idx]

        with torch.no_grad():
            if use_dot:
                scores = mashup_batch @ api_embeddings.T
            elif hasattr(trainer.scorer, "score_all"):
                scores = trainer.scorer.score_all(
                    mashup_batch, api_embeddings
                )
            else:
        ...
        for row, mashup_id in enumerate(ids):
            rankings[mashup_id] = [
                int(api_id) for api_id in topk[row].tolist()
            ]

        print(
            f"[Ranking] {min(start + batch_size, len(target_mashup_ids))}"
